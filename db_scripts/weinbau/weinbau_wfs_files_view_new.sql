...
AS SELECT psf.fid,
  ...
  WHERE psfo.username::text = current_setting('my.wfsuser'::text, true) OR psfo.username::text = 'gast'::text OR current_setting('my.wfsuser'::text, true) = 'geoserver-admin'::text AND psfo.owner = true;
